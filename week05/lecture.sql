...
CREATE OR REPLACE EXTERNAL TABLE `data_lake.phl_opa_properties_second` (
    `geog` STRING,
...
OPTIONS (
  ...
  uris = ['gs://bairun_data_lake/phl_opa_properties/*.jsonl'],
  format = 'JSON',
  max_bad_records = 0
)

CREATE OR REPLACE TABLE `data_lake.phl_opa_properties_new`
CLUSTER BY (geog)
AS (
  SELECT * EXCEPT(geog), st_geogfromgeojson(geog) AS geog
  -- OR SELECT * REPLACE (st_geogfromgeojson(geog) AS geog)
  FROM `data_lake.phl_opa_properties_second`
)

-- BigQuery
SELECT * REPLACE(ST_GEOGFROMTEXT(geog) AS geog) FROM `learn-cloud-with-mjumbe.data_lake.phl_opa_properties`